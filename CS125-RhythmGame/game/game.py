...
import os
import random
import queue  # Add queue module import
...
class Game:
    ...
    def update(self):
        # If paused or showing results, don't update game state
        if self.paused or self.show_results:
            return
    ...
    def pause_game(self):
        if not self.paused:  # Only pause if not already paused
            self.paused = True
            # Store both absolute pause time and elapsed time
            self.pause_absolute_time = pygame.time.get_ticks()
            self.pause_elapsed_time = self.pause_absolute_time - self.start_ticks
            
            print(f"[DEBUG] Pausing game at elapsed time: {self.pause_elapsed_time/1000.0}s")
            
            # Store current game state
            self.pause_game_state = {
                'elapsed_time': self.pause_elapsed_time / 1000.0,  # Convert to seconds
                'arrow_count': len(self.arrow_group),
                'queue_state': self.arrow_spawner.get_queue_state(),
                'start_ticks': self.start_ticks  # Store the original start_ticks
            }
            
            # Pause music if it's playing
            try:
                if self.music_started and pygame.mixer.music.get_busy():
                    pygame.mixer.music.pause()  # Use pause instead of stop
            except Exception as e:
                print(f"[ERROR] Failed to pause music: {e}")
                
            # Pause video if it's active
            if self.background_video and self.background_video.active:
                self.video_position = self.background_video.get_playback_data()['time']
                self.background_video.toggle_pause()
            
            # Pause arrow spawning
            self.arrow_spawner.spawning_allowed = False
    ...
    def resume_game(self):
        if self.paused:  # Only resume if currently paused
            ...
            current_time = pygame.time.get_ticks()
            pause_duration = current_time - self.pause_absolute_time
            
            # Restore the original start_ticks and add the pause duration
            self.start_ticks = self.pause_game_state['start_ticks'] + pause_duration
            
            print(f"[DEBUG] Resuming game at elapsed time: {self.pause_game_state['elapsed_time']}s")
            print(f"[DEBUG] Arrow count at pause: {self.pause_game_state['arrow_count']}")
            print(f"[DEBUG] Pause duration: {pause_duration/1000.0}s")
            
            # Resume music if it was playing
            try:
                if self.music_started and hasattr(self, 'music_position'):
                    pygame.mixer.music.unpause()  # Use unpause instead of restarting
            except Exception as e:
            ...
            if self.background_video and not self.background_video.active:
                try:
                    ...
                    if not self.background_video.active:
                        self.background_video.restart()
                    # Seek to the correct position
                    self.background_video.seek(self.video_position)
                    self.background_video.toggle_pause()
                except Exception as e:
                    print(f"[ERROR] Failed to resume video: {e}")
            
            # Resume arrow spawning
            self.arrow_spawner.spawning_allowed = True
            
            # Update arrow spawner timing with the stored elapsed time
            if hasattr(self, 'pause_game_state'):
                # Restore the spawn queue state
                self.arrow_spawner.restore_queue_state(self.pause_game_state['queue_state'])
                
                # Update timing and ensure all future arrows are preserved
                self.arrow_spawner.update_timing(self.pause_game_state['elapsed_time'])
            
            # Clear any stored frames to prevent visual artifacts
            self.pause_frame = None
