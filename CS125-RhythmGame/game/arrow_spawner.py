...
class ArrowSpawner:
    ...
    def spawn_arrow(self, current_time, arrow_group, gravity_mode=False):
        ...
        if not self.spawn_queue.empty():
            item = self.spawn_queue.queue[0]
            # Changed condition to allow spawning if timestamp is in the future
            if item >= current_time and item - current_time <= SPAWN_WINDOW:
                timestamp = round(self.spawn_queue.get(), 3)
    ...
    def update_timing(self, elapsed_time):
        """
        Update timing after pause to maintain synchronization.
        
        This method:
        1. Adjusts spawn queue timestamps
        2. Updates arrow positions
        3. Maintains game rhythm
        
        Args:
            elapsed_time: The elapsed game time in seconds
        """
        print(f"[DEBUG] Updating timing with elapsed time: {elapsed_time}")
        
        # Create a new queue with adjusted timestamps
        new_queue = queue.Queue()
        temp_timestamps = []
        
        # First, collect all timestamps and their associated keys
        while not self.spawn_queue.empty():
            timestamp = self.spawn_queue.get()
            temp_timestamps.append(timestamp)
        
        # Sort timestamps to maintain order
        temp_timestamps.sort()
        print(f"[DEBUG] All timestamps before filtering: {temp_timestamps}")
        
        # Add back ALL timestamps that are in the future
        for timestamp in temp_timestamps:
            # Keep all timestamps that are in the future relative to elapsed time
            if timestamp >= elapsed_time:
                new_queue.put(timestamp)
                # Ensure the key mapping is preserved
                if timestamp in self.timestamp_key_dict:
                    self.timestamp_key_dict[timestamp] = self.timestamp_key_dict[timestamp]
                    print(f"[DEBUG] Preserving timestamp {timestamp} with keys {self.timestamp_key_dict[timestamp]}")
        
        self.spawn_queue = new_queue
        print(f"[DEBUG] Queue size after update: {self.spawn_queue.qsize()}")

    def get_queue_state(self):
        """Get the current state of the spawn queue and timestamp dictionary."""
        timestamps = []
        temp_queue = queue.Queue()
        
        # Store all timestamps while preserving the queue
        while not self.spawn_queue.empty():
            timestamp = self.spawn_queue.get()
            timestamps.append(timestamp)
            temp_queue.put(timestamp)
        
        # Restore the queue
        self.spawn_queue = temp_queue
        
        print(f"[DEBUG] Saving queue state with {len(timestamps)} timestamps")
        print(f"[DEBUG] Timestamps: {timestamps}")
        print(f"[DEBUG] Timestamp dictionary: {self.timestamp_key_dict}")
            
        return {
            'timestamps': timestamps,
            'timestamp_dict': self.timestamp_key_dict.copy()
        }

    def restore_queue_state(self, state):
        """Restore the spawn queue and timestamp dictionary from a saved state."""
        print(f"[DEBUG] Restoring queue state with {len(state['timestamps'])} timestamps")
        print(f"[DEBUG] Timestamps to restore: {state['timestamps']}")
        print(f"[DEBUG] Timestamp dictionary to restore: {state['timestamp_dict']}")
        ...
        # Restore timestamp dictionary
        self.timestamp_key_dict = state['timestamp_dict'].copy()
        
        print(f"[DEBUG] Queue size after restore: {self.spawn_queue.qsize()}")
        print(f"[DEBUG] Current timestamp dictionary: {self.timestamp_key_dict}")
